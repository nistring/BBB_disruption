...
def quantify_vol(subject, prognosis):
    ...
    plot_stat_map(
        ...
        threshold=threshold,
        title=f"{subject}/{prognosis}/ne",
        radiological=True,
        display_mode="z",
    )
    mean_pv = contrast_mask.sum() / gm.sum()
    return mean_pv
...
if __name__ == "__main__":
    ...
    def do(sub, prog):
        for s, p in zip(sub, prog):
            FLIRT(s)
            percentage_vol = quantify_vol(s, "good" if p == 1 else "poor")
            print(s, p, percentage_vol)
            ...
            outcome.loc[int(s), "volume(%)"] = percentage_vol * 100
